...
export async function getBootstrapData(orgId: string): Promise<BootstrapData> {
  ...
  const [departmentsResult, locationsResult, categoriesResult] = await Promise.all([
    supabase
      .from('departments')
...
export async function getMyTickets(
  ...
  const tickets = (data || []).map((item: any) => ({
    ...
    location: item.locations ? { name: item.locations.name } : null,
    category: item.categories ? { name: item.categories.name } : null,
  }));
